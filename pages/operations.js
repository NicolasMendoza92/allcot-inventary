    ...
    if (operationSearched.length !== "") {
      searchedoperations = operations.filter((op) => {
        return (
          op.transaction
            .toLowerCase()
            .includes(operationSearched.toLowerCase()) ||
          ...
          op.projectData?.idProject
            .toLowerCase()
            .includes(operationSearched.toLowerCase()) ||
          op.projectData?.standardOp
            .toLowerCase()
            .includes(operationSearched.toLowerCase()) ||
          op.projectData?.nameProject
            .toLowerCase()
            .includes(operationSearched.toLowerCase()) ||
    ...
    return (
      <div className="flex justify-center">
        <div className="shadow-md p-3 bg-zinc-300/10 flex items-center gap-2 m-3">
          <h1>You must be logged in to see operations</h1>
          <button
          ...
          <tbody>
            ...
            {filteredOperations.map((op) => (
              <tr key={op._id}>
                ...
                {op.delivery === "Done" && (
                  <td>
                    <b style={{ color: "green", fontSize: "15px" }}>Done</b>
                  </td>
                )}
                {op.delivery === "Pending" && (
                  <td>
                    <b style={{ color: "red", fontSize: "15px" }}>Pending</b>
                  </td>
                )}
                {op.payment === "Done" && (
                  <td>
