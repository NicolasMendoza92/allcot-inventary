    ...
    if (clientSearched.length !== "") {
      customerserch = clients.filter((cl) => {
        return (
          cl.nombreCliente
            .toLowerCase()
            .includes(clientSearched.toLowerCase()) ||
          cl.mainContact
            ?.toLowerCase()
            .includes(clientSearched.toLowerCase()) ||
          cl.paisCliente
      ...
      {session && (
        <>
          {session?.user?.email === "mdo@karbon-x.com " ? (
            <div className=" text-center"> You are not allowed to see the customers</div>
          ) : (
            <>
                ...
                <table className=" basic my-3">
                  ...
                  <tbody>
                    ...
                    {clientsFinded?.map((c) => (
                      <tr key={c._id}>
                        <td>
                          {new Date(c.createdAt).toLocaleString("GB-English", {
                            dateStyle: "short",
                          })}
                        </td>
