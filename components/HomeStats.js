...
export default function HomeStats({ operations }) {
  ...
  const operationsToday = operations.filter(
    (op) => new Date(op.createdAt) > subHours(new Date(), 24)
  );
  const operationsWeek = operations.filter(
    (op) => new Date(op.createdAt) > subHours(new Date(), 24 * 7)
  );
  const operationsMonth = operations.filter(
    (op) => new Date(op.createdAt) > subHours(new Date(), 24 * 30)
  );

  const openDelivery = operations.filter((op) => op.delivery === "Pending");
  const openPayment = operations.filter((op) => op.payment === "Pending");
  const operationsClosed = operations.filter(
    (op) => op.payment === "Done" && op.delivery === "Done"
  );
  ...
  const creditsSold = salesOp
    .map((o) => o.quantity)
    .reduce((count, o) => count + parseFloat(o), 0);
  const creditsPurchased = purchaseOp
    .map((o) => o.quantity)
    .reduce((count, o) => count + parseFloat(o), 0);
  const pendingDelivery = openDelivery
    .map((o) => o.quantity)
    .reduce((count, o) => count + parseFloat(o), 0);
      ...
      <div className="board-grid">
        <div className="board-card-link" onClick={goToOps}>
          ...
          <div className="board-desc">
            You have {openDelivery.length} pending deliveries.{" "}
          </div>
        </div>
        <div className="board-card-link" onClick={goToOps}>
          ...
          <div className="board-desc">
            You have {openPayment.length} pending payments.
          </div>
        </div>
        <div className="board-card">
          ...
          <div className="board-desc">
            You close {operationsClosed.length} operations.
          </div>
        </div>
      </div>
      ...
      <div className="board-grid">
        <div className="board-card">
          ...
          <div className="board-number">
            {creditsSold.toLocaleString("es-ES")}
          </div>
        ...
        <div className="board-card">
          ...
          <div className="board-number">
            {creditsPurchased.toLocaleString("es-ES")}
          </div>
        ...
        <div className="board-card">
          ...
          <div className="board-number">
            {pendingDelivery.toLocaleString("es-ES")}
          </div>
          <div className="board-desc">
            from {openDelivery.length} operations
          </div>
        </div>
      </div>
    </div>
